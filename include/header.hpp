...
void fillup(const path &path) {
  for (directory_entry &x : directory_iterator(path)) {
    ...
    brokers[broker_name][account].first =
        max(brokers[broker_name][account].first, date);
